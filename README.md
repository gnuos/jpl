...
- 📍 **更好的错误消息**：运行时错误显示行号 + 源码上下文（箭头标记 + 前后代码行），便于快速定位

### 性能优化

- 🔄 **尾调用优化 (TCO)**：自递归尾调用栈帧复用，支持无限深度递归不溢出
...
- 🖥️ **REPL 界面**：交互式命令行，支持自动补全、历史记录、多行续输、`:doc` 函数签名查询
...
- 📍 **更好的错误消息**：运行时错误显示行号 + 源码上下文（箭头标记 + 前后代码行）
...
# 复杂代码（使用引号）
jpl eval 'for ($i = 0; $i < 5; $i += 1) { print $i }'
```
...
#   :funcs        - 显示所有内置函数
#   :consts       - 显示预设常量
#   :doc <name>   - 查看函数签名（参数、返回值、描述）
#   :quit         - 退出 REPL

# REPL 多行续输
#   输入未闭合的括号或引号时，自动进入多行模式
#   提示符变为 '... '，输入空行提交代码
